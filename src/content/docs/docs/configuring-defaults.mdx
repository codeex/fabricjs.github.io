...
利用配置FabricJs的全局默认属性，来满足你的自定义环境需求。

### 配置对象的默认控制

如果你正在寻找如何全局配置默认控制操作，请查看另外一篇文章[配置控制操作](/docs/configuring-controls)。
...
- 你不喜欢对象周围一圈1px的透明描边
- 你希望所有的文本对象使用自定义字体
- 你想自己设置 ```originX``` 和 ```originY``` 坐标作为程序的中心
...
配置默认值而不是在每个对象实例时设置每个属性可以减少代码中的混乱，但从性能的角度来看，这样做并不会更快。
...
如果你真的喜欢旧版本的方法，你仍然可以通过添加一些代码来切换使用。
