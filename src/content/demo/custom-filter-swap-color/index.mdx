...
在滤镜类中，`applyTo2d` 是您需要查看的函数，用于支持标准的非 WebGL 模式。  
如果您在浏览器中运行，这段代码通常不需要，只有在运行在 Node 环境时才会有用。不过，FabricJS 现在同时支持这两种模式，因此，如果您想编写一个滤镜，最好同时处理这两种模式。
...
如果您对 WebGL 熟悉，这部分内容会显得非常简单；如果不熟悉的话，需要掌握一些概念。  
**免责声明**：我并不擅长 WebGL，这段代码反映了 FabricJS 如何将基本功能封装成可复用的组件。

首先，您需要设置 WebGL 着色器中的变量注入，这是通过 `getUniformLocations` 和 `sendUniformData` 两个函数完成的。  
`getUniformLocations` 用来定义这些变量在着色器中的名称，而 `sendUniformData` 则负责将数据绑定到这些变量上。编写 `sendUniformData` 显然是最困难的部分，因为您必须了解您使用的变量类型。在我们的例子中，我们正在将颜色字符串转换为一个介于 0 和 1 之间的 4 个数字的数组，为此我们需要使用 `gl.uniform4fv` 函数。
